...
class Zht:
    ...
    def segment(self):
        ...
        for linet in fit:
            ...
            tok_pos = ["{}".format(x) for x,y in zip(tokens, pos_tags)]  # list of tok_pos pairs
...
class Zhs:
    ...
    def segment(self):
        ...
        for line in fi:
            ...
            n += 1
            if (line.strip() == ''): # empty string
                fo.write("\n")
                continue
            # segment with POS-tagging
            line_seg = pynlpir.segment(line, pos_tagging=False) 
            fo.write(sep.join(line_seg) + "\n")
            #if (n == 1): break
            if n % 50 == 0:
                self.window.ui.progressBar.setValue(round(100 * n / self.fi_linecount, 0))
                self.window.ui.progressBar.repaint()
...
class Zhs_pos:
    ...
    def segment(self):
        ...
        for line in fi:
            ...
            # segment with POS-tagging
            try:
                line_seg = pynlpir.segment(line, pos_tagging=True, pos_names=None) 
                for (w, p) in line_seg:
                    if p == None: p = 'None' 
                    fo.write(w + '_' + p + sep)
                fo.write("\n")
